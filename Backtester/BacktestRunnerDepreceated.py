...
class BacktestRunner:
    ...
    def _open_position(self, time: datetime, row: pd.Series, side: str):
        price, atr_value = row.get("next_open", row["close"]), row.get("atr")
    ...
    def _handle_exits(
        ...
        """More realistic exit handling with slippage and gaps"""
        if not self.position.is_open:
            return False

        exit_price, exit_reason = None, None

        # Check for gap at open (most critical for stops)
        if self.position.side == "long":
            if self.position.sl_price and open_price <= self.position.sl_price:
                # Gapped down through stop - execute at open with slippage
                slippage = self._calculate_slippage(open_price, gap=True)
                exit_price = max(open_price - slippage, 0)  # Can't go negative
                exit_reason = "Stop Loss (Gap)"
            elif self.position.tp_price and open_price >= self.position.tp_price:
                slippage = self._calculate_slippage(open_price, gap=True)
                exit_price = open_price - slippage  # Slippage works against you
        ...
        # If no gap, check if price hit during the candle
        if exit_reason is None:
            if self.position.side == "long":
                if self.position.sl_price and low_price <= self.position.sl_price:
                    exit_price = self.position.sl_price
                    exit_reason = "Stop Loss"
                elif self.position.tp_price and high_price >= self.position.tp_price:
                    exit_price = self.position.tp_price
                    exit_reason = "Take Profit"
            else:  # short
                if self.position.sl_price and high_price >= self.position.sl_price:
                    exit_price = self.position.sl_price
                    exit_reason = "Stop Loss"
                elif self.position.tp_price and low_price <= self.position.tp_price:
                    exit_price = self.position.tp_price
                    exit_reason = "Take Profit"

        if exit_reason:
            # Apply slippage if not already applied
    ...
    def _calculate_slippage(self, price: float, gap: bool = False) -> float:
        """Calculate realistic slippage based on market conditions"""
        # Basic slippage model - can be enhanced with volume/volatility
        base_slippage = price * 0.0001  # 1 basis point
        if gap:
            base_slippage *= 3  # Higher slippage on gaps
